...
exports.getHomeShop = (req, res, next) => {
  let newProducts, newFoodAndDrink, newMeals, newConsumerGoods;
  ...
  Product.find()
    ...
    .then((products) => {
      ...
      return Product.find({
        $or: [
          { type: '65086762dba20b246f1cd839' },
          { type: '6508654838d412e34d4d4866' },
        ],
      })
        .sort({ createdAt: -1 })
        .limit(4);
    })
    .then((foodAndDrinkProducts) => {
      newFoodAndDrink = foodAndDrinkProducts;

      return Product.find({ type: '650867d1dba20b246f1cd869' })
        .sort({ createdAt: -1 })
        .limit(4);
    })
    .then((meals) => {
      ...
      return Product.find({ type: '6509794ebe62d2f8e2c55b93' })
        .sort({ createdAt: -1 })
        .limit(4);
    })
    .then((consumerGoods) => {
      newConsumerGoods = consumerGoods;
      res.render('shop/home', {
        ...
        newFoodAndDrink: newFoodAndDrink,
      });
    })
    .catch((err) => {
      console.log(err);
